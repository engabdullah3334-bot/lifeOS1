"""
Task Manager - Port from LifeOS_Integrated
Handles all task-related operations with JSON persistence
"""
...
CATEGORY = ["daly", "weekly", "monthly", "yearly", "not_time"]
PRIORITY = ["low", "medium", "high", "critical"]
STATUS = ["pending", "in_progress", "completed", "delayed"]


class Task:
    def __init__(self, task_id, title, description, due_date, category, priority, 
                 status="pending", created_at=None, start_time=None, end_time=None, 
                 recurrence=None, estimated_time=None, actual_time=None, notes=None):
...
class TaskManager:
    ...
    def get_productivity_percentage(self):
        """Calculate productivity based on completed vs total tasks for today"""
        today_tasks = [t for t in self.tasks.values() 
                      if t.due_date and t.due_date.date() == datetime.now().date()]
        if not today_tasks:
            return 0
        completed = sum(1 for t in today_tasks if t.status == "completed")
        return int((completed / len(today_tasks)) * 100)
